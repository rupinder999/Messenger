...
//    @IBOutlet weak var imageView: UIImageView!
    ...
    @objc private func registerButtonTapped(){
        guard let email = emailField.text, let password = passwordField.text, let firstName = firstNameField.text, let lastName = lastNameField.text, !email.isEmpty, !password.isEmpty, !firstName.isEmpty, !lastName.isEmpty, password.count >= 6 else{
            alertUserLoginError()
            return
        }
        ...
        DatabaseManager.shared.userExists(with: email, completion: { [weak self] exists in
            ...
            guard !exists else{
                //user already exists
                strongSelf.alertUserLoginError(message: "Looks like an account for that email address already exists.")
                return
            }
